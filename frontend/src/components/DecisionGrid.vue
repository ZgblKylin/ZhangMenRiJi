...
const props = defineProps<{ decisions: Decision[]; arts: MartialArt[]; game: GameState; used: string[]; title?: string }>()
...
const available = (d: Decision) => props.game.decisions_used < props.game.max_decisions && !props.game.pending_event && !props.used.includes(d.id) && !(d.costType === 'silver' && props.game.silver < d.cost) && !(d.id === 'train' && props.game.injury >= 30) && !(['study', 'research'].includes(d.id) && !hasUnlearnedPlayerArt()) && !(d.id === 'rest' && props.game.injury <= 5)

...
const loadGame = (id: string) => loading(async () => { try { const data = await gameApi.get(id); gameId.value = data.id; G.value = { ...data.state, sect_name: data.sect_name || data.state.sect_name || '' }; usedDecisions.value = []; ui.savePanel = false } catch (e) { report('载入失败', e) } })
...
const advance = () => loading(async () => { if (!gameId.value || !G.value) return; try { const name = G.value.sect_name; const data = await gameApi.advance(gameId.value); G.value = { ...data.state, sect_name: data.state.sect_name || name }; usedDecisions.value = []; if (data.events?.length || data.tournament) { ui.popupEvents = data.events || []; ui.tournament = data.tournament || null; ui.popup = true } } catch (e) { report('推进失败', e) } })
const closePopup = () => { ui.popup = false; ui.popupEvents = []; ui.tournament = null }
const restart = () => { if (confirm('确定重开山门？')) resetGame() }
const keyboard = (event: KeyboardEvent) => { if (event.key === 'Escape') { if (ui.popup) closePopup(); else ui.savePanel = false } else if (ui.popup && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); closePopup() } }
...
<template>
  <ScrollContainer>
    ...
    <GameView v-else :game="G" :decisions="DECISIONS" :arts="MARTIAL_ARTS" :used="usedDecisions" @saves="openSaves('manage')" @restart="restart" @decide="decide" @advance="advance" />
  </ScrollContainer>

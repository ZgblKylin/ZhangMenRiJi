...
pub fn execute_decision(
    ...
    match decision_id {
        ...
        "train" => {
            ...
            events.push(GameEvent {
                text: "掌门率众苦练一月，弟子内力均有所长。掌门略感疲惫。".into(),
                mood: "good".into(),
                year: state.year,
                month: state.month,
                category: "sect".into(),
        ...
        "mission" => {
            ...
            if let Some(d) = state.disciples.iter_mut().filter(|d| d.alive).nth(idx) {
                ...
                if score > 25 {
                    ...
                    events.push(GameEvent {
                        text: format!("{}下山行侠，仗剑惩恶，带回银{}两，乡民称颂。", d.name, sg),
                        mood: "good".into(),
                        year: state.year,
                        month: state.month,
                        category: "sect".into(),
                    ...
                    state.injury = clamp(state.injury + disc::rand_range(rng, 3, 8), 0, 100);
                    events.push(GameEvent {
                        text: format!("{}下山行侠，不料遭遇强敌！弟子受伤，掌门亲赴接应。", d.name),
                        mood: "bad".into(),
                        year: state.year,
                        month: state.month,
                        category: "sect".into(),
        ...
        "repair" => {
            ...
            events.push(GameEvent {
                text: "山门修缮一新，弟子们很是高兴，练功也格外卖力。".into(),
                mood: "good".into(),
                year: state.year,
                month: state.month,
                category: "sect".into(),
        ...
        "diplomacy" => {
            ...
            events.push(GameEvent {
                text: format!("掌门携礼拜访邻派，相谈甚欢。{}", extra),
                mood: "good".into(),
                year: state.year,
                month: state.month,
                category: "sect".into(),
        ...
        "study" => {
            ...
            if !unlearned.is_empty() && rng.gen_bool(0.5) {
                ...
                events.push(GameEvent {
                    text: "掌门闭关苦思，虽未创出新招，但对武学之理解更深了一层。".into(),
                    mood: "neutral".into(),
                    year: state.year,
                    month: state.month,
                    category: "sect".into(),
        ...
        "teach" => {
            ...
            events.push(GameEvent {
                text: format!("掌门亲自为{}传功，诸弟子获益良多。", names.join("、")),
                mood: "good".into(),
                year: state.year,
                month: state.month,
                category: "sect".into(),
